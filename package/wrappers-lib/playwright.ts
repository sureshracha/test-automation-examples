...
export class UiElement {
    ...
    protected async getElements(options?: { index?: number }) {
        return await this.waitTillElementToBeReady().then(async () => {
            ...
            let ele = await this.getHasFrame() ? this.page.frameLocator(this.stringFramelocator).locator(await this.getLocator()).all() : this.page.locator(await this.getLocator()).all();
            // console.log('found element: ', ele['_selector']);
